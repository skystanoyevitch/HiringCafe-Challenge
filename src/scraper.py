...
def scrape_job_description(detail_url, base_domain):
    """Smart URL handling - works with both full URLs and relative paths"""
...
def main():
     get_bloomberg_url = "https://bloomberg.avature.net/careers/SearchJobs"
     base_domain = "https://bloomberg.avature.net"
     ...
     html = fetch_page(get_bloomberg_url)
     if html is None:
     ...
     for page in range(total_pages):
         if page == 0:
             ...
             page_offset = page * 12
             page_html = fetch_page(f"{get_bloomberg_url}/?jobRecordsPerPage=12&jobOffset={page_offset}")
